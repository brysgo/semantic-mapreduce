...
class Rule {
  ...
  dependencies() {
    if (this._dependencies == null) {
      ...
      if (params) {
        this._dependencies = Array.from(params).map(p =>
          this.constructor.get(p)
        );
      }
      if (
        this._dependencies == null ||
        Array.from(this._dependencies).includes(undefined)
      ) {
        this._dependencies = [];
      }
    }
  ...
  passes(dependencies) {
    ...
    for (let dependency of Array.from(dependencies)) {
      if (!Array.from(this._passes).includes(dependency)) {
        return -1;
      }
    }
